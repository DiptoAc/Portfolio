    ...
    <>
      <section id="projects" className="py-12 sm:py-16 md:py-20 bg-gradient-to-br from-slate-950 via-slate-950 to-slate-900 relative overflow-hidden">
        ...
        <div className="container mx-auto px-6 relative z-10">
          ...
          {/* </div><div className="max-w-[90rem] mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8"> */}
          {/* <div className="max-w-[90rem] mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-8"> */}
          <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-16">
            ...
            return (
              <motion.div
                ...
                className={`bg-gradient-to-br from-slate-800/50 to-slate-700/50 rounded-xl shadow-lg hover:shadow-2xl transition-all cursor-pointer relative overflow-hidden group border border-slate-600/30 border-t-4 ${project.color || 'border-t-sky-500'}`}
              >
                {/* 1. Project Title (Now at the top) */}
                <div className="pt-6 px-6 pb-4 relative z-10">
                  <h3 className={`text-xl sm:text-2xl font-semibold text-slate-100 transition-colors ${colors.hoverText}`}>
                    {project.title}
                  </h3>
                </div>

                {/* 2. Project Image (Below Title) */}
                {project.image && (
                  <div className="relative h-48 w-full overflow-hidden border-y border-slate-700/30">
                    <img
                      src={project.image}
                      alt={project.title}
                ...
                {/* 3. Content Container (Description, Tech, Links) */}
                <div className="p-6 relative z-10">
                  ...
                  <div className="flex flex-wrap gap-2 mb-6">
                    {project.tech.map((tech, techIndex) => (
                      <span 
                        key={techIndex} 
                        className="px-3 py-1 bg-slate-900/50 text-sky-300 text-xs sm:text-sm rounded-full border border-slate-600/30 backdrop-blur-md"
                      >
                        {tech}
                      </span>
                    ))}
                  ...
                  <div className="flex gap-4 items-center">
                    ...
                    <span className={`text-xs ml-auto font-medium transition-colors text-slate-400 ${colors.hoverText}`}>
                      {project.customRoute ? "Case Study →" : "Details"}
                    </span>
                  </div>
                </div>
